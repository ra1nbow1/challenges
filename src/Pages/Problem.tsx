...
function Problem() {
				...
				if (status) {
					...
					setOutput(
						response.data.traceback ||
							'Не удалось запустить код. Исправьте синтаксические ошибки.'
					)
				}
			})
	}
	...
	const check = (): void => {
		...
		if (
			...
			setOutput(
				'Исправьте название функции или ее аргументы. Попробуйте перенести модули в функцию.'
			)
		}
	}
		...
		<div className={styles.wrapper}>
			<div className={styles.left}>
				...
				<div className={styles.buttons}>
					<button
						...
						onClick={() => setOpenedTab('instructions')}
						style={
							openedTab == 'instructions'
								? { background: '#343131' }
								: { background: 'none' }
						}>
						Условие
			...
			{!isMobile ? (
				<div className={styles.right}>
					...
					<div className={styles.row}>
						<div className={styles.warning}>
							Для корректной работы не изменяйте название и аргументы функции
							solution().
						</div>
				...
				<div className={styles.mobile_warning}>
					Решение задач не предусмотрено на мобильных устройствах.
				</div>
			)}
		</div>
	)
